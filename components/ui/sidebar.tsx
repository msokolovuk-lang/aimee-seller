            ...
            {items.map((item) => {
              ...
              return (
                <Link
                  key={item.id}
                  href={item.href}
                  className={`relative flex items-center gap-2.5 px-3 py-2.5 rounded-aimee-sm text-[13px] mb-0.5 transition-all ${isActive ? 'bg-teal-dim text-teal font-semibold' : 'text-txt-secondary hover:bg-teal-dim hover:text-txt'}`}
                >
                  {isActive && (
                  ...
                  {item.badge && (
                    <span className={`ml-auto text-[9px] font-mono font-semibold px-1.5 py-0.5 rounded-full ${badgeStyles[item.badgeColor || 'teal']}`}>
                      {item.badge === 'live' ? (
